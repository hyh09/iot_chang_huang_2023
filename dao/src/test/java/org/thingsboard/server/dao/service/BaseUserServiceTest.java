...
public abstract class BaseUserServiceTest extends AbstractServiceTest {
    ...
    public void before() {
        ...
        tenantAdmin.setEmail("tenant@huansi.net");
        ...
        customerUser.setEmail("customer@huansi.net");
    ...
    public void testFindUserByEmail() {
        User user = userService.findUserByEmail(SYSTEM_TENANT_ID, "sysadmin@huansi.net");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.SYS_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "tenant@huansi.net");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.TENANT_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "customer@huansi.net");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.CUSTOMER_USER, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "fake@huansi.net");
    ...
    public void testSaveUser() {
        User tenantAdminUser = userService.findUserByEmail(SYSTEM_TENANT_ID,"tenant@huansi.net");
        User user = new User();
        user.setAuthority(Authority.TENANT_ADMIN);
        user.setTenantId(tenantAdminUser.getTenantId());
        user.setEmail("tenant2@huansi.net");
    ...
    public void testSaveUserWithSameEmail() {
        User tenantAdminUser = userService.findUserByEmail(tenantId, "tenant@huansi.net");
        tenantAdminUser.setEmail("sysadmin@huansi.net");
    ...
    public void testFindTenantAdmins() {
        ...
        for (int i = 0; i < 124; i++) {
            ...
            user.setEmail("testTenant" + i + "@huansi.net");
    ...
    public void testFindTenantAdminsByEmail() {
        ...
        for (int i = 0; i < 94; i++) {
            ...
            String email = email1 + suffix + "@huansi.net";
        ...
        for (int i = 0; i < 132; i++) {
            ...
            user.setTenantId(tenantId);
            String suffix = RandomStringUtils.randomAlphanumeric((int) (5 + Math.random() * 10));
            String email = email2 + suffix + "@huansi.net";
            email = i % 2 == 0 ? email.toLowerCase() : email.toUpperCase();
            user.setEmail(email);
    ...
    public void testFindCustomerUsers() {
        User customerUser = userService.findUserByEmail(tenantId, "customer@huansi.net");
        ...
        for (int i = 0; i < 156; i++) {
            ...
            user.setEmail("testCustomer" + i + "@huansi.net");
