...
public interface DictDataRepository extends PagingAndSortingRepository<DictDataEntity, UUID>, JpaSpecificationExecutor<DictDataEntity> {
    ...
    @Query("select d from DictDataEntity d where d.tenantId = :tenantId order by d.created_time desc")
    List<DictDataEntity> findAllByTenantId(@Param("tenantId") UUID tenantId);

    @Query("select d from DictDataEntity d where d.tenantId = :tenantId and d.name in :keys order by d.created_time desc")
