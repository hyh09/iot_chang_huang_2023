...
public class CapacityDeviceConfigController extends BaseController {
     ...
     * 日期: 2022-03-01
     * ###############################
     * bug: ts_kv中查询不到数据；但是此接口仍有数据?
     * ***** select *  from  ts_kv  where  entity_id='0048d391-6465-11ec-903a-0b302b886cc7' and ts>='1580486400000' and ts<='1675871999000' and "key"='1241'
     * *解决方法： （测试数据问题)
     * select min(ts)  from  ts_kv ; -- 1664582400000  ;最早的数据是 2022-10-01 08:00:00 ，之前的数据都被清理过，但是hs_energy_hour 表的数据没有被清理
     * delete   from hs_energy_hour  where ts<1664582400000
     * ###############################
     *
     * @param deviceId
     * @param startTime
